...
export async function runReport() {
  ...
  const results = await Promise.allSettled(
    properties.map((prop) =>
      fetchPropertyReport(client, prop.id, { startDate, endDate, prevStartDate, prevEndDate })
    )
  );
  ...
  const reportProps = results.map((result, i) => {
  ...
  return { propertiesProcessed, propertiesFailed };
}
